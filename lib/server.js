...
const installedFacade = (installed) => {

  if (!installed) return null
  return installed.map(inst => Object.assign({}, inst, {
    containers: inst.containers.map(c => c.Id) 
  }))
}
...
const dockerFacade = (docker) => {
  ...
  if (docker.data) {
    facade = Object.assign({}, facade, docker.data, { 
      installed: installedFacade(docker.computed.installed)
    })
  }
