 ...
 */
function udevinfo(sysfspath, done) {
...
function udevinfo_attrwalk(sysfspath, done){
...
module.exports = (paths, done) => {
  ...
  // FIXME should validate paths, and done
	function pathIteratee(path, done) {

		let tasks = [	done => udevinfo(path, done),
								done => udevinfo_attrwalk(path, done) ]
	...
	async.map(paths, pathIteratee, (e, r) => done(e, r)) 
}
