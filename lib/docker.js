...
function appStatus(recipeKeyString) {
  ...
  if (state.docker === null || 
      ...
      (!state.docker.computed.installed))
  { return 'UNAVAIL' }

  let installed = state.docker.computed.installed

  let inst = installed.find(i => i.recipeKeyString === recipeKeyString)
...
async function containerDeleteCommand(id) {
  ...
  if (!docker || !docker.computed || !docker.computed.installed) return null

  let installed = docker.computed.installed

  console.log('>>>>')
  installed.forEach(inst => console.log(inst.containers))
  console.log('<<<<')

  let inst = installed.find(i => {
