...
describe(path.basename(__filename), () => {
  ...
  describe('preset directory uuid, empty', () => {
    ...
    it('should throw ENOTDIR if target path is a file', async () => {
      let fpath = path.join(tmpdir, 'hello')
      await fs.writeFileAsync(fpath, 'world')
      let xstat1 = await readXstatAsync(fpath)
      let w = probe(fpath, xstat1.uuid, FILE.NULLTIME, 0)
      w.on('error', (err, again) => {
        expect(again).to.be.false
        expect(err).to.be.an.instanceof(E.ENOTDIR)
      })
      w.on('finish', (data, again) => { throw new Error('error finish') })
      w.start()
    })

    it('should throw EINSTANCE when force probing directory with different uuid', done => {
    ...
    it('should return null data if mtime match', done => {
      ...
      w.on('error', (err, again) => { throw new Error('error error') })
    ...
    it('should return xstat list if mtime renewed', done => {
      fillAsync(tmpdir, tree01).asCallback((err, data1) => {
        // console.log(data1.mtime)
        // console.log(xstat.mtime)
    ...
    it('again should be true if request when probing', done => {
      let w = probe(tmpdir, xstat.uuid, xstat.mtime, 100)
      w.on('error', (err, again) => { 
        throw new Error('error error')})
      w.on('finish', (data, again) => {
        expect(again).to.be.true
        done()
      })
      w.start()
      setTimeout(() => w.request(), 10)
    })

    it('should return error if request when probe finished', done => {
      let w = probe(tmpdir, xstat.uuid, xstat.mtime, 0)
      w.on('error', (err, again) => { throw new Error('error error')})
      w.on('finish', (data, again) => {
        expect(data).to.be.null
