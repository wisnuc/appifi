...
import FileData from '../../../src/fruitmix/file/fileService'
...
const createTestTrees = async (model, fileData) => {
  let n1, n2, n3, n4, n5, n6, n7, n8
   model.emit('drivesCreated', [ {uuid: UUIDMap.get('uuid1'), type: 'private',owner: UUIDMap.get('userUUID')}, 
                                 {uuid: UUIDMap.get('uuid9'), type: 'private', owner: UUIDMap.get('aliceUUID')},
                                 {uuid: UUIDMap.get('uuid10'), type: 'public', writelist: [UUIDMap.get('userUUID')], readlist: [UUIDMap.get('bobUUID')], shareAllowed: false},
                                 {uuid: UUIDMap.get('uuid11'), type: 'public', writelist: [UUIDMap.get('aliceUUID')], readlist: [UUIDMap.get('bobUUID')], shareAllowed: true}
                               ])
    ...
    fileData.createNode(n1, {type: 'directory', uuid: UUIDMap.get('uuid2'), name: 'n2'})
    await Promise.delay(100)
    n2 = fileData.uuidMap.get(UUIDMap.get('uuid2'))
    // console.log(n2.parent.name)
    fileData.createNode(n2, {type: 'directory', uuid: UUIDMap.get('uuid3'), name: 'n3'})
    await Promise.delay(100)
    n3 = fileData.uuidMap.get(UUIDMap.get('uuid3'))
    // console.log(n3.parent.name)
    fileData.createNode(n3, {type: 'directory', uuid: UUIDMap.get('uuid4'), name: 'n4'})
    await Promise.delay(100)
    n4 = fileData.uuidMap.get(UUIDMap.get('uuid4'))
    // console.log(n4.parent.name)
    fileData.createNode(n1, {type: 'directory', uuid: UUIDMap.get('uuid5'), name: 'n5'})
    await Promise.delay(100)
    n5 = fileData.uuidMap.get(UUIDMap.get('uuid5'))
    // console.log(n5.parent.name)
    fileData.createNode(n1, {type: 'directory', uuid: UUIDMap.get('uuid6'), name: 'n6'})
    await Promise.delay(100)
    n6 = fileData.uuidMap.get(UUIDMap.get('uuid6'))
    // console.log(n6.parent.name)
    fileData.createNode(n6, {type: 'directory', uuid: UUIDMap.get('uuid7'), name: 'n7'})
    await Promise.delay(100)
    n7 = fileData.uuidMap.get(UUIDMap.get('uuid7'))
    // console.log(n7.parent.name)
    fileData.createNode(n7, {type: 'directory', uuid: UUIDMap.get('uuid8'), name: 'n8'})
    await Promise.delay(100)
    n8 = fileData.uuidMap.get(UUIDMap.get('uuid8'))
}
