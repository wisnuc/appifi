...
const NFS = require('src/fruitmix/NFS')
...
describe(path.basename(__filename), () => {
  ...
  describe('non-api tests', () => {
    it('constructor should throw if opts is null', done => {
      expect(() => new NFS(null)).to.throw()
      done()
    })

    it('constructor should throw if volumeUUID is 123', done => {
      expect(() => new NFS({ volumeUUID: 123 })).to.throw()
      done()
    })

    it('update w/ 10-disk storage should have 3 volume and 6 partition drives', done => {
      let nfs = new NFS({ volumeUUID: UUIDBC })
      nfs.update(storage)
      let keys = nfs.drives.map(drv => drv.uuid ? drv.uuid : drv.name)
      expect(keys.sort()).to.deep.equal([UUIDDE, UUIDF, 
        'sdi1', 'sdi2', 'sdi3', 'sdi5', 'sdi6', 'sdi7'].sort())
      done()
    })
  })
