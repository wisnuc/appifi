...
module.exports = {
  init: () => { throw new Error('HubUtils.init triggered') } 
  /*
  function () {
  ...
  setPrompted: function (prompted) {
    ...
    throw new Error('setPrompted called')
  ...
  request: function (req, callback) {
    ...
    request(req, (error, response, body) => {
      ...
      if (data && data.detail && data.detail.indexOf('expired') !== -1) {
        ...
        let username = this.creds(config)[0]
        let password = this.creds(config)[1]
  ...
  logout: function () {
    throw new Error('hubutils logout called')
  ...
  login: function (username, password, callback) {
    ...
    this.auth(username, password, (error, response, body) => {
      if (error) {
        throw new Error('hubutils login called')
