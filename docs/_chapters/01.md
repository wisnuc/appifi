...
Fruitmix uses linux `xattr` (file extended attributes) to store extra metadata for files and folders. 

```json

{
  "uuid": <string> each file or folder is assigned to a uuid (v4).
  "writelist": <uuid string array>, users who have the write permission for this folder or file.
  "readlist": <uuid string array>, users who have the read permission for this folder or file. 
  "hash": sha256 hash of the file
  "htime": hash timestamp, used to check if hash valid
  "magic": a number for uninterested version or a string, currently only JPEG is supported.
}
```
