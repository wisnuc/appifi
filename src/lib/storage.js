...
This module provides a bunch of storage-related functions.

Most of them are not `thread-safe`, they should be wrapped and synchronized before servicing other modules.
...
const probeFruitmix = async mountpoint => {
  ...
  try {
    ...
    if (e.code === 'ENOENT' || e.code === 'ENOTDIR') { throw e } else if (e.code === 'EISDIR' || e instanceof SyntaxError) { throw Object.assign(new Error('' + e.code + ':' + e.message), { code: 'EDATA' }) } else { throw Object.assign(new Error('' + e.code + ':' + e.message), { code: 'EFAIL' }) }
  }
}
...
const refresh = callback => refreshAsync()
  .then(storage => callback(null, storage))
  .catch(err => callback(err))

const mkfsBtrfs = callback => mkfsBtrfs()
