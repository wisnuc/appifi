...
class FieldHandler2 extends EventEmitter {

  constructor(ctx, part, props) {
    super()
    ...
    part.on('end', () => {
      ...
      try {
        Object.assign(this, JSON.parse(Buffer.concat(this.buffers)))
        this.schedule()
      } catch (e) {
        this.emit('finish', e)
      }
  ...
  schedule() {
    if (this.blocked || this.part) return
    if (this.op === 'mkdir') {
      let fruit = getFruit()
      let { user, driveUUID, dirUUID } = this.ctx
      fruit.mkdirp(user, driveUUID, dirUUID, this.toName, (err, xstat) => {
        if (err) {
          this.emit('finish', err)
        } else {
          this.emit('finish', null, xstat)
        }
      })
    } 
  }
...
class FileHandler2 extends EventEmitter {
...
class Writedir extends EventEmitter {
  ...
  push(part, props) {
    if (!part.filename) {
      let handler = new FieldHandler2(this, part, props)  
      handler.on('finish', (err, data) => {
        let index = this.queue.indexOf(handler)
        this.queue[index] = { err, data }
  ...
  schedule() {
    // is handler
    const f = x => x instanceof FieldHandler2 || x instanceof FileHandler2

    if (this.ended && this.queue.every(x => !f(x))) {
      this.emit('finish')
    } else {
      this.queue.filter(f).reduce((names, h) => {
        if (!names.includes(h.fromName)) h.unblock()
        if (!names.includes(h.toName)) names.push(h.toName)
        return names
      }, [])
    } 
  } 
