...
class Pending {
  ...
  */
  ...
  read(handler) {
    ...
    if (Number.isInteger(handler))
      this.timer = setTimeout(() => this.dir.readdir = new Working(this.dir), handler)
    else if (typeof handler === 'function' || handler instanceof Monitor)
      this.dir.readdir = new Working(this.dir, [handler])
    else if (handler === undefined) 
      this.dir.readdir = new Working(this.dir, [])
...
class Working {
  ...
  */
  constructor(dir, handlers) {
    ...
    */
    this.compareTimestamp = handlers ? false : true
  ...
  finish(err, xstats, mtime, transient) {
    ...
    if (err) {
      ...
      if (mtime !== this.dir.mtime) 
        this.dir.merge(xstats, this.handlers.filter(h => h instanceof Monitor))

      if (mtime !== this.dir.mtime && transient === false) 
  ...
  */
  readXstats(callback) {
    ...
    fs.readdir(this.dirPath, (err, entries) => {
      ...
      let xstats = [], count = entries.length
  ...
  start() {
    ...
    readXstat(this.dirPath, (err, xstat1) => {
      ...
      if (xstat1.type !== 'directory') 
        return this.finish(Object.assign(new Error('not a dir'), { code: 'ENOTDIR'}))
      if (xstat1.uuid !== this.dir.uuid) 
        return this.finish(Object.assign(new Error('uuid mismatch'), { code: 'EINSTANCE'}))
  ...
  */
  read(handler) {
    ...
    if (handler instanceof Monitor) 
      handler.start(this.dir.uuid, this.dir.name)
